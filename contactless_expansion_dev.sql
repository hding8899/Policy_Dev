...
create or replace table risk.test.risk_score_simu as(
    ...
    where 1=1
    and rae.auth_event_created_ts::date between '2022-07-01' and '2022-08-31'/*for vol estimation and justification*/
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
user_id - usage info: application/cta dvc related
...
width_bucket(NUNIQUE__TIMEZONES_P2H, 0, 10, 10) as bin
, min(NUNIQUE__TIMEZONES_P2H) as min_val, max(NUNIQUE__TIMEZONES_P2H) as max_val
, count(*) as cnt_txn
...
select 
case 
     when final_amt>=50 and risk_score>=30 and max_atom_score_p30>=0.3 and  (NUNIQUE__DEVICE_IDS_P5M>=2 or NUNIQUE__DEVICE_IDS_P2H-NUNIQUE__DEVICE_IDS_P5M>=2 or NUNIQUE__DEVICE_IDS_P1D-NUNIQUE__DEVICE_IDS_P2H>=2) then 'dec 1.1'
    
     when final_amt>=50 and MAX_ATOM_SCORE_P2H>=0.5 /*or CNT_DIST_DVC_P2H>=3*/ then 'dec 1.2'
...
case when final_amt>=200 and max_atom_score_p1d>=0.1 and  (count__email_change_by_user_p7d-count__email_change_by_user_p5m>=1)   then 'dec 1.1'
     ...
     when final_amt>=200 and MAX_ATOM_SCORE_P1D>=0.4 and (NUNIQUE__TIMEZONES_P2H-NUNIQUE__TIMEZONES_P5M>=2   
